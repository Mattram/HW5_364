...
## TODOdone 364: Create a database in postgresql in the code line below, and fill in your app's database URI. It should be of the format: postgresql://localhost/YOUR_DATABASE_NAME

## Your final Postgres database should be your uniqname, plus HW5, e.g. "jczettaHW5" or "maupandeHW5"
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:password@localhost/mattramHW5"
## Provided:
...
# TODOdone 364: Define an UpdateButtonForm class for use to update todo items
...
# TODOdone 364: Define a DeleteButtonForm class for use to delete todo items
...
# TODOone 364: Update the all_lists.html template and the all_lists view function such that there is a delete button available for each ToDoList saved.
...
# TODOdone 364: Update the one_list view function and the list_tpl.html view file so that there is an Update button next to each todolist item, and the priority integer of that item can be updated. (This is also addressed in later TODOs.)
...
# TODOdone 364: Fill in the update_item.html template to work properly with this update route. (HINT: Compare against example!)

# TODOdone 364: Complete route to delete a whole ToDoList
@app.route('/delete/<lst>',methods=["GET","POST"])
def delete(lst):
    form = DeleteButtonForm()
    if form.validate_on_submit():
        d = TodoList()
        d = TodoList.query.get(lst)
        db.session.delete(d)
        db.session.commit()
